...
func embedSubtitles(stepParam *types.SubtitleTaskStepParam, isHorizontal bool) error {
	...
	// 检测系统是否支持NVIDIA GPU编码
	var useGPU bool
	
	// 首先检查GPU设备是否可用
	gpuCheckCmd := exec.Command("nvidia-smi")
	gpuOutput, gpuErr := gpuCheckCmd.CombinedOutput()
	if gpuErr != nil {
		log.GetLogger().Warn("nvidia-smi命令失败，GPU可能不可用，将使用CPU编码", 
			zap.Error(gpuErr))
	} else {
		log.GetLogger().Info("检测到NVIDIA GPU", zap.String("nvidia-smi输出", string(gpuOutput)))
